# CassKube
[![Gitter](https://badges.gitter.im/casskube/community.svg)](https://gitter.im/casskube/community?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
[![Go Report Card](https://goreportcard.com/badge/github.com/khulnasoft/casskube)](https://goreportcard.com/report/github.com/khulnasoft/casskube)
[![License: Apache License 2.0](https://img.shields.io/github/license/khulnasoft/casskube)](https://github.com/khulnasoft/casskube/blob/master/LICENSE.txt)

The KhulnaSoft Kubernetes Operator for Apache Cassandra&reg;
...
kubectl create -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/docs/user/casskube-manifests-v1.16.yaml
kubectl create -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/operator/k8s-flavors/gke/storage.yaml
kubectl -n casskube create -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/operator/example-cassdc-yaml/cassandra-3.11.x/example-cassdc-minimal.yaml
```

### Loading the operator

Installing the CassKube itself is straightforward. We have provided manifests for each Kubernetes version from 1.15 through 1.19. Apply the relevant manifest to your cluster as follows:

```console
K8S_VER=v1.16
kubectl apply -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/docs/user/casskube-manifests-$K8S_VER.yaml
```
...
This will deploy the operator, along with any requisite resources such as Role, RoleBinding, etc., to the `casskube` namespace. You can check to see if the operator is ready as follows:

```console
$ kubectl -n casskube get pods --selector name=casskube
NAME                             READY   STATUS    RESTARTS   AGE
casskube-555577b9f8-zgx6j   1/1     Running   0          25h
```
...
```
kubectl apply -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/operator/k8s-flavors/gke/storage.yaml
```
...
```console
kubectl -n casskube apply -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/operator/example-cassdc-yaml/cassandra-3.11.x/example-cassdc-minimal.yaml
```

You can check the status of pods in the Cassandra cluster as follows:

```console
$ kubectl -n casskube get pods --selector cassandra.khulnasoft.com/cluster=cluster1
NAME                         READY   STATUS    RESTARTS   AGE
...
```console
$ kubectl -n casskube get cassdc/dc1 -o "jsonpath={.status.cassandraOperatorProgress}"
Ready
```
...
```console
$ kubectl -n casskube exec -it -c cassandra cluster1-dc1-default-sts-0 -- nodetool status
Datacenter: dc1
===============
Status=Up/Down
...
$ CASS_USER=$(kubectl -n casskube get secret cluster1-superuser -o json | jq -r '.data.username' | base64 --decode)
$ CASS_PASS=$(kubectl -n casskube get secret cluster1-superuser -o json | jq -r '.data.password' | base64 --decode)
$ kubectl -n casskube exec -ti cluster1-dc1-default-sts-0 -c cassandra -- sh -c "cqlsh -u '$CASS_USER' -p '$CASS_PASS'"
...
# Helm 2
helm install khulnasoft/casskube
...
```console
kubectl create namespace casskube-system
helm install --namespace=casskube-system casskube ./charts/casskube-chart
```
...
clusterRoleName: casskube-cr
clusterRoleBindingName: casskube-crb
roleName: casskube
roleBindingName: casskube
webhookClusterRoleName: casskube-webhook
webhookClusterRoleBindingName: casskube-webhook
deploymentName: casskube
deploymentReplicas: 1
defaultImage: "khulnasoft/casskube:1.6.0"
...
```console
kubectl create namespace casskube-system
helm install --set clusterWideInstall=true --namespace=casskube-system casskube ./charts/casskube-chart
```
...
```console
helm install --set image=docker.pkg.github.com/khulnasoft/casskube/operator:latest-ubi --set imagePullSecrets=github-docker-registry casskube ./charts/casskube-chart
```
...
* The config builder init container, built from sources in [khulnasoft/cass-config-builder](https://github.com/khulnasoft/cass-config-builder).
* Cassandra, built from
  [khulnasoft/management-api-for-apache-cassandra](https://github.com/khulnasoft/management-api-for-apache-cassandra),
  with Cassandra 3.11.7 support, and experimental support for Cassandra
  4.0-beta1.
* ... or DSE, built from [khulnasoft/docker-images](https://github.com/khulnasoft/docker-images).

### Overriding properties of casskube created Containers

If the CassandraDatacenter specifies a podTemplateSpec field, then containers with specific names can be used to override default settings in containers that will be created by casskube.

Currently casskube will create an InitContainer with the name of "server-config-init". Normal Containers that will be created have the names "cassandra", "server-system-logger", and optionally "reaper". 

In general, the values specified in this way by the user will override anything generated by casskube.

Of special note is that user-specified environment variables, ports, and volumes in the corresponding containers will be added to the values that casskube automatically generates for those containers.
...
As of version 1.0, CassKube is maintained by a team at KhulnaSoft and it is
part of what powers [KhulnaSoft
Astra](https://www.khulnasoft.com/cloud/khulnasoft-astra). We would love for open
...
```
kubectl delete -f https://raw.githubusercontent.com/khulnasoft/casskube/v1.6.0/docs/user/casskube-manifests-v1.16.yaml
```

## Contacts

For development questions, please reach out on [Gitter](https://gitter.im/casskube/community), or by opening an issue on GitHub.

For usage questions, please visit our Community Forums: https://community.khulnasoft.com
