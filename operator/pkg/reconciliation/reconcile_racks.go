...
import (
	...
	"github.com/khulnasoft/casskube/operator/internal/result"
	api "github.com/khulnasoft/casskube/operator/pkg/apis/cassandra/v1beta1"
	"github.com/khulnasoft/casskube/operator/pkg/events"
...
func (rc *ReconciliationContext) desiredStatefulSetForExistingStatefulSet(sts *appsv1.StatefulSet, rackName string) (desiredSts *appsv1.StatefulSet, err error) {
	...
	// when CassKube was released, we accidentally used the incorrect managed-by
	// label of "cassandra-operator" we have since fixed this to be "casskube",
...
// Checks to see if any node is starting. This is done by checking to see if the cassandra.khulnasoft.com/node-state label
// has a value of Starting. If it does then check to see if the C* node is ready. If the node is ready, the pod's
// cassandra.khulnasoft.com/node-state label is set to a value of Started. This function returns two bools and an error.
// The first bool is true if there is a C* node that is Starting. The second bool is set to true if a C* node has just
// transitioned to the Started state by having its cassandra.khulnasoft.com/node-state label set to Started. The error is
