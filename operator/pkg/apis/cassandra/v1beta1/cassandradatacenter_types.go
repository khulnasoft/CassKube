...
const (
	...
	ClusterLabel = "cassandra.khulnasoft.com/cluster"

	// DatacenterLabel is the operator's label for the datacenter name
	DatacenterLabel = "cassandra.khulnasoft.com/datacenter"

	// SeedNodeLabel is the operator's label for the seed node state
	SeedNodeLabel = "cassandra.khulnasoft.com/seed-node"

	// RackLabel is the operator's label for the rack name
	RackLabel = "cassandra.khulnasoft.com/rack"

	// RackLabel is the operator's label for the rack name
	CassOperatorProgressLabel = "cassandra.khulnasoft.com/operator-progress"

	// PromMetricsLabel is a service label that can be selected for prometheus metrics scraping
	PromMetricsLabel = "cassandra.khulnasoft.com/prom-metrics"

	// CassNodeState
	CassNodeState = "cassandra.khulnasoft.com/node-state"
...
type CassandraDatacenterSpec struct {
	...
	// AdditionalServiceConfig allows to define additional parameters that are included in the created Services. Note, user can override values set by casskube and doing so could break casskube functionality.
	// Avoid label "casskube" and anything that starts with "cassandra.khulnasoft.com/"
