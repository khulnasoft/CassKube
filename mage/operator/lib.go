...
import (
	...
	gitutil "github.com/khulnasoft/casskube/mage/git"
...
const (
	...
	cassOperatorTarget         = "casskube"
	cassOperatorUbiTarget      = "casskube-ubi"
	...
	generatedDseDataCentersCrd = "operator/deploy/crds/cassandra.khulnasoft.com_cassandradatacenters_crd.yaml"
	helmChartCrd               = "charts/casskube-chart/templates/customresourcedefinition.yaml"
	packagePath                = "github.com/khulnasoft/casskube/operator"
...
func generateK8sAndOpenApi() {
	cwd, _ := os.Getwd()
	runArgs := []string{"-t", "--rm"}
	repoPath := "/go/src/github.com/khulnasoft/casskube"
	execArgs := []string{
		"/bin/bash", "-c",
...
func runGoBuild(version string) {
	...
	binaryPath := fmt.Sprintf("../build/bin/casskube-%s-%s", runtime.GOOS, runtime.GOARCH)
	goArgs := []string{
		"build", "-o", binaryPath,
...
func doGenerateClient() {
	...
	execArgs := []string{"client", "github.com/khulnasoft/casskube/operator/pkg/generated",
		"github.com/khulnasoft/casskube/operator/pkg/apis", "cassandra:v1beta1"}
	volumes := []string{fmt.Sprintf("%s:/go/src/github.com/khulnasoft/casskube", cwd)}
