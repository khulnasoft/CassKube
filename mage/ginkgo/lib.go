...
func (ns NsWrapper) Terminate() {
	...
	// out there
	_, helmErrOut, helmErr := helm_util.UninstallCapture("casskube", ns.Namespace)
...
func (ns *NsWrapper) WaitForDatacenterToHaveNoPods(dcName string) {
	...
	json := "jsonpath={.items}"
	k := kubectl.Get("pods").
		WithLabel(fmt.Sprintf("cassandra.khulnasoft.com/datacenter=%s", dcName)).
		FormatOutput(json)
...
func (ns *NsWrapper) WaitForPodNotStarted(podName string) {
	...
	k := kubectl.Get("pod").
		...
		WithFlag("selector", "cassandra.khulnasoft.com/node-state=Started")
...
func (ns *NsWrapper) WaitForCassandraImages(dcName string, expectedImages []string, timeout int) {
	...
	k := kubectl.Get("pods").
		WithFlag("selector", fmt.Sprintf("cassandra.khulnasoft.com/datacenter=%s", dcName)).
		FormatOutput(json)
...
func HelmInstallWithOverrides(chartPath string, namespace string, overrides map[string]string) {
	...
	err := helm_util.Install(chartPath, "casskube", namespace, overrides)
	mageutil.PanicOnError(err)
}
...
func HelmUpgradeWithOverrides(chartPath string, namespace string, overrides map[string]string) {
	...
	err := helm_util.Upgrade(chartPath, "casskube", namespace, overrides)
	mageutil.PanicOnError(err)
}
